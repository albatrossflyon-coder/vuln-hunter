...
- 2026-09-05 — **A semgrep process timeout in `run_scan()` now returns partial findings, not nothing.** History: the call had a hardcoded process timeout that `raise`d `RuntimeError` on `TimeoutExpired`, so a timeout on a large repo returned zero findings ("scan came back empty"). Fixed 2026-09-05: on timeout the scanner parses semgrep's partial JSON (`e.stdout`), returns those findings plus a synthetic `semgrep.timeout` LOW marker, and never raises for the timeout case. Both timeouts are env-configurable: `SEMGREP_INTERNAL_TIMEOUT` (per-rule, default 30), `SEMGREP_PROCESS_TIMEOUT` (process, default 1800). If a scan still looks short, check for the `semgrep.timeout` marker in the results and bump `SEMGREP_PROCESS_TIMEOUT`, or run `semgrep.exe` directly via `run_in_background` with no imposed timeout, same configs vuln-hunter uses.
...
- 2026-09-05 — **There is no pytest suite.** `backend/venv` has no `pytest` installed; `backend/test_*_manual.py` are standalone scripts run directly (`python test_foo_manual.py`), each printing PASS/FAIL and `sys.exit`ing. Scan-only ones (`test_never_read_manual.py`, `test_exclude_dirs_manual.py`) need semgrep but no API key; triage/pipeline ones need the Groq key. Don't reach for `pytest -q` — it just errors "No module named pytest."
